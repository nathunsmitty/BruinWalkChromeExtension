...
function findClassPlanClasses() {
  ...
  for (var i = 0; i < uclaClasses.length; i++) {
    ...
    var classNumber = classinfo[1].innerHTML.substring(
      0,
...
function getInstSearchRes(bwalkButton) {
  ...
  chrome.runtime.sendMessage(
    // JSON that's sent
    {
      ...
      action: "xhttp",
      url:
        "http://www.bruinwalk.com/search/?category=professors&q=" + uriInstName,
      data: ""
    },
    ...
    function(responseHTML) {
      if (responseHTML != "error") {
        ...
        for (var i = 0; i < allNamesFromRes.length; i++) {
          ...
          var instLastNameFromRes = instNameFromRes
            .replace(/[.,\/#!$%\^&\*;:'{}=\-_`~()]/g, "")
            .match(/\s(\w+)$/)[0]
            .replace(/\s/g, "");
          var instFirstInFromRes = instNameFromRes.replace(
            /[.,\/#!$%\^&\*;:'{}=\-_`~()]/g,
            ""
          )[0];
...
function instButtEvLis(instBut) {
  return function() {
    ...
    var popupContainers = instBut.parentElement.getElementsByClassName(
    ...
    if (popupContainers.length == 0) {
        ...
        chrome.runtime.sendMessage(
          ...
          function(responseHTML) {
            if (responseHTML != "error") {
              ...
              titleDiv.append(
                "Showing Bruin Walk results for: " +
                  instBut.getAttribute("found-tag")
              );
              popup.appendChild(titleDiv);

              // Calls another function to create the ratings table for this professor
              var instRatingTable = createGenInstRatingTable(
      ...
      //   if professor wasnt found at bruinwalk
        ...
        instBut.href =
          "http://www.bruinwalk.com/search/?category=professors&q=" +
          encodeURI(instBut.getAttribute("inst-name"));
    ...
    // this also makes the popup appear when the button is mouseovered
      ...
      var popups = instBut.parentElement.getElementsByClassName(
        "inst-rating-popup-cont"
      );
...
function createClassGradeDist(instBut) {
  var instPageUrl =
    ...
    instBut
      ...
      .toLowerCase() +
    "/" +
    instBut.getAttribute("course-name").toLowerCase() +
    "/";
  ...
  chrome.runtime.sendMessage(
    ...
    function(responseHTML) {
      ...
      if (allScores.length > 0) {
        ...
        var scoresString = allScores[0].getElementsByClassName(
        ...
        for (var i = 0; i < scoresString.length; i++) {
          ...
          bar.style.height =
            String(Math.floor((scores[i] / maxScore) * 30)) + "px";
          bar.style.width = "5px";
          bar.className = "grades-dist-bar";

          var topBar = document.createElement("div");
          topBar.style.height =
            String(35 - Math.floor((scores[i] / maxScore) * 30)) + "px";
