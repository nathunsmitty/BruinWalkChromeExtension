...
function findClassPlanClasses() {
    ...
    for (let i = 0; i < uclaClasses.length; i++) {
        ...
        for (let j = 0; j < instCont.length; j++) {
            if (instCont[j].tagName == 'TD' && j % 2) { addInstButtons(instCont[j], className) }
        }
    }
}
        ...
        (responseHTML) => {
            ...
            && instructorName.substring(instructorName.indexOf(' ') + 1)[0]
              == instFirstInFromRes
                        ...
                        if (score < 2) { bwalkButton.className = 'inst-button-bwalk inst-rating-bad' } else if (score > 3.5) { bwalkButton.className = 'inst-button-bwalk inst-rating-good' }

                        const seeMoreLink = tempDiv
                            .getElementsByClassName('sr-info')
                            [i].getElementsByClassName('see-more')[0]
                        const instPageUrl = `http://www.bruinwalk.com${
    ...
    // If popover hasn't been loaded, load it
        ...
        if (popupContainers.length == 0) {
            ...
            //   if professor wasnt found at bruinwalk
                ...
                instBut.href = `http://www.bruinwalk.com/search/?category=professors&q=${
...
function createClassGradeDist(instBut) {
    const instPageUrl = `http://www.bruinwalk.com/professors/${
        instBut
...
function instButtEvLisMOut(instBut) {
    return function () {
        instBut.parentElement.getElementsByClassName(
            'inst-rating-popup-cont',
        )[0].className = 'inst-rating-popup-cont hide popover clickover fade bottom in'
